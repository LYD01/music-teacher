...
/**
 * Converts a note name (with sharps or flats) + octave to a MIDI note number.
 * Handles enharmonic equivalents so "Bb4" and "A#4" both return 70.
 */
export function noteNameToMidi(name: string, octave: number): number {
	const pitchClass = PITCH_CLASS_MAP[name];
